...
module.exports = async function (context, req) {
    ...
    const result = await client.database("handson").container("follow_users")
        .items.query(query).fetchAll();
    
    context.log(`Cosmos DB result: ${JSON.stringify(result)}`);

    const msgs = result.resources.map(e => ({user_id: e.user_id, follow_id: e.follow_id, timestamp: e.timestamp}));
